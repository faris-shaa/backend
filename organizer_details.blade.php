...
@section("content")
    ...
    <div class="container flex flex-col items-center justify-center" id="organizerProfile">
        ...
        <div class="flex flex-row gap-5 md:gap-7 mb-3 justify-center">
            @if(optional($organizer->organizerDetails)->facebook)
                <a href="{{ optional($organizer->organizerDetails)->facebook }}" target="_blank">
            ...
            @if(optional($organizer->organizerDetails)->whatsapp)
                <a href="{{ optional($organizer->organizerDetails)->whatsapp }}" target="_blank">
            ...
            @endif
            @if(optional($organizer->organizerDetails)->twitter)

                <a href="{{ optional($organizer->organizerDetails)->twitter }}" target="_blank">
            ...
            @endif
            @if(optional($organizer->organizerDetails)->instagram)
                <a href="{{ optional($organizer->organizerDetails)->instagram }}" target="_blank">
                    <svg width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    @if(((app()->getLocale() == "عربي") ? optional($organizer->organizerDetails)->long_description_arabic : optional($organizer->organizerDetails)->long_description_english))
        <div class="container mt-12 md:mt-32">
            <div class="bg-light bg-opacity-5 rounded-2xl border border-primary_color_o10_1  p-2  md:p-32-24 mt-4 lg:mt-8"
                 id="aboutOrganizer">
                <h3 class="text-primary_color_6 font-medium text-h4 lg:text-h3">{{ __('About') . " ". ((app()->getLocale() == "عربي" and optional($organizer->organizerDetails)->arabic_name) ? optional($organizer->organizerDetails)->arabic_name :  $organizer["organization_name"]) }}</h3>
                <p class="h5 lg:h4 mt-4 paragraph-3 max-h-96 overflow-x-auto ">
                    {{(app()->getLocale() == "عربي") ? optional($organizer->organizerDetails)->long_description_arabic : optional($organizer->organizerDetails)->long_description_english }}
                </p>
            </div>
        </div>
    @endif
