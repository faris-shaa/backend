...
class FrontendController extends Controller
{
    ...
    public function home()
    {
        ...
        if (env('DB_DATABASE') == null) {
            ...
            return view('front.home2', compact('events', 'organizer', 'category', 'blog', 'banner', 'user', 'showLinkBanner', 'language', 'pervious_events'));
        }
    }
    ...
    public function userResetPassword(Request $request)
    {
        ...
        if (!$user) {
            // Return JSON response for AJAX requests, otherwise normal redirect
            if ($request->ajax()) {
                return response()->json(['success' => false, 'error' => 'Invalid Email Id, Please try another.']);
            } else {
        ...
        if ($user) {
            ...
            if ($request->ajax()) {
                return response()->json(['success' => true, 'message' => 'New password will send in your mail, please check it.']);
            } else {
    ...
    public function allEvents(Request $request)
    {
        ...
        return view('front.events', compact('user', 'events', 'chip', 'onlinecount', 'offlinecount'));
    }
    ...
    public function eventDetail($id, $name = null)
    {
        ...
        foreach ($data->paid_ticket as $value) {
            if ($data->is_repeat == 1) {
                ...
                $used = Order::where('ticket_id', $value->id)->where('created_at', '<', $end_date)->where('created_at', '>', $start_date)->where('payment_status', 1)->where('order_status', 'Complete')->sum('quantity');
                //dd($start_date , $end_date);
                $total_paid = $total_paid + $value->quantity;
                $used_paid = $used_paid + $used;
            } else {
                $used = Order::where('ticket_id', $value->id)->where('payment_status', 1)->where('order_status', 'Complete')->sum('quantity');
                $total_paid = $total_paid + $value->quantity;
                $used_paid = $used_paid + $used;
            }
        }
        ...
        foreach ($data->paid_ticket as $value) {
            ...
            if($id == 258 )
            {
                ...
                $used = OrderChild::where('ticket_id', $value->id)->whereIn('order_id',$event_orders)->count();
            }
            $value->available_qty = $value->quantity - $used;
        }
        //dd($data->paid_ticket);
        ...
        $minPrice = $event->paid_ticket->min('price');

        return view('front.eventDetail', compact('currency', 'lang', 'event', 'images', 'tags', 'appUser', 'rate', 'ticket_detail', "minPrice"));
    }
...
//            Session::put('event_id', $id);
        ...
        $quantities = explode(',', $quantitiesString);*/
            ...
            foreach ($tickets as $key => $value) {
                ...
                if ($value['quantity'] > 0) {
                    ...
                    $ticket_amount += $ticket->price * $value['quantity'];

                    // Calculate used quantity and available quantity
                    $used = Order::where('ticket_id', $value['ticket_id'])->sum('quantity');
                    $available_qty = $ticket->quantity - $used;
                    ...
                    $taxes = Tax::where([
                        ['allow_all_bill', 1],
                        ['status', 1]
                    ])->orderBy('id', 'DESC')->get()->makeHidden(['created_at', 'updated_at']);

                    // Calculate tax amounts
                    foreach ($taxes as $item) {
                        if ($item->amount_type == 'percentage') {
                            $amount = (($item->price * $ticket->price) / 100) * $value['quantity'];
                    ...
                    $tax_total = $tax_total + round(array_sum($arr), 2);

                    // Calculate total tax percentages and prices
                    $totalPersTax += Tax::where([
                        ['allow_all_bill', 1],
                        ['status', 1],
                        ['amount_type', 'percentage']
                    ])->sum('price');

                    $totalAmountTax += Tax::where([
                        ['allow_all_bill', 1],
                        ['status', 1],
                        ['amount_type', 'price']
                    ])->sum('price');
                    $ticket->selected_quantity = $value['quantity'];
            ...
            $data->totalAmountTax = $totalAmountTax;
            $data->ticket_per_order = 3;
            $data->available_qty = 4;
            $data->price = 4;
            $data->type = 'paid';
            $data->id = 4;
            $data->name = 4;
           ...
           foreach ($tickets as $key => $ticket_array) {
               ...
               {   
                   ...
                   if (($sold_ticket_count + $ticket_array['quantity']) > $ticekt->quantity && $event->is_repeat == 0) {
                       ...
                       Log::info("sold out--44");
                       return response()->json([
                           'error_message' => 'Tickets Sold out',
                           'type' => 'ApplePay',
                           'status' => 500
                       ]);
                   }
                   ...
                   $order_id_check_count = Order::where([['order_status', 'Complete']])->where('order_status', "Complete")->where('payment_status', 1)->pluck('id')->toArray();
                   $sold_ticket_count = OrderChild::whereIn('order_id', $order_id_check_count)->where('ticket_id', $ticekt->id)->count();
                   if (($sold_ticket_count + $ticket_array['quantity']) > $ticekt->quantity && $event->is_repeat == 0) {
                       
                       Log::info("sold out--55");
                       return response()->json([
                           'error_message' => 'Tickets Sold out',
                           'type' => 'ApplePay',
                           'status' => 500
               ...
               if($event->is_repeat == 1 )
               {
                   ...
                   $order_id_check_count = Order::where([['order_status', 'Complete']])->where('order_status', "Complete")->where('payment_status', 1)->pluck('id')->toArray();
                   $sold_ticket_count = OrderChild::whereIn('order_id', $order_id_check_count)->where('ticket_id', $ticekt->id)->where('time_slot_id',$request->time_slot_id)->where('event_book_date',$request->slot_event_date)->count();
                   if (($sold_ticket_count + $ticket_array['quantity']) > $ticekt->quantity) {
                       
                       Log::info("sold out--66");
                       return response()->json([
                           'error_message' => 'Tickets Sold out',
                           'type' => 'ApplePay',
                           'status' => 500
    ...
    //     // Extract the ticket IDs and quantities
    //     $ticketIdsString = $queryParams['ticket_ids'] ?? null;
    //     $quantitiesString = $queryParams['ticket_quantities'] ?? null;

    //     if (!$ticketIdsString || !$quantitiesString) {
    //         // Handle the case where ticket_ids or ticket_quantities are missing
    //         return response()->json(['error' => 'Ticket IDs or quantities are missing.'], 400);
    //     }
    ...
    //     $tickets = array_map(function ($ticketId, $quantity) {
    //         return [
    //             'ticket_id' => (int) $ticketId,  // Casting to int
    //             'quantity' => (int) $quantity,   // Casting to int
    //         ];
    //     }, $ticketIds, $quantities); 
    ...
    //         $quantities = 0;
    //         foreach($tickets as $key => $value){
    //             $quantities = $quantities + @$value['quantity'];
    //         } 
    //         foreach($tickets as $key => $value){
    //             $ticket = Ticket::findOrFail($value['ticket_id']);
    //             DB::table('order_tickets')->insert([
    //                 'order_id' => @$order->id,
    //                 'ticket_id' => @$value['ticket_id'],
    //                 'quantity' => @$value['quantity'],
    //                 'amount' => $ticket->price * $value['quantity'],
    //             ]);
    ...
    public function sendOrderMailPdfSms($id)
    {

        $order = Order::with(['customer'])->find($id);

        $event = Event::where('id', $order->event_id)->first();
        $org_user = User::where('id', $event->user_id)->first();


        $to = "509108875";  // str_replace('+', '', $org_user->phone);
        $message = "عزيزي " . $org_user->first_name . " ،
تم حجز تذكرة جديدة لفعاليتك    " . $event->name . " من قبل المستخدم  " . $order->customer->name . " يرجى مراجعة منصة المنظم لمتابعة الحجوزات.";
        ...
        if (true) {
            try {
                ...
                curl_setopt_array($curl, array(
                    ...
                    CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                    CURLOPT_CUSTOMREQUEST => 'POST',
                    CURLOPT_POSTFIELDS => $json_data,  // Send the properly encoded JSON data
                    CURLOPT_HTTPHEADER => array(
                        'Content-Type: application/json; charset=UTF-8',  // Ensure UTF-8 content type
                        'Authorization: Bearer 17bcd048f6bad60a6812030bd1c1c5c2'
                    ),
                ));
    ...
    public function sendOrderMailPdf($id)
    {
        ...
        $event = Event::where('id', $order->event_id)->first();
        $org_user = User::where('id', $event->user_id)->first();
        // if($org_user && $org_user->phone )
        // {
        //     $otp = rand(100000, 999999);

        //     $to = $org_user->phone ;  // str_replace('+', '', $org_user->phone);
        //     $message = "عزيزي ".$org_user->first_name ."،م حجز تذكرة جديدة لفعاليتك ".$event->name." من قبل المستخدم ".$order->customer->first_name.". يرجى مراجعة منصة المنظم لمتابعة الحجوزات.";
        //     if (true) {
...
تم حجز تذكرة جديدة لفعاليتك    " . $event->name . " من قبل المستخدم  " . $order->customer->name . " يرجى مراجعة منصة المنظم لمتابعة الحجوزات.";
        ...
        // sent sms to Mr. ghrom
        $to = "565009999";  // str_replace('+', '', $org_user->phone);
        $message = "يوجد حجز على فعالية  " . $event->name . " 
من قبل المستخدم   " . $order->customer->name;
...
من قبل المستخدم   " . $order->customer->name;
    ...
    public function allCategory()
    {
        ...
        return view('front.allCategories', compact('data', 'catactive'));
    }
...
// dd("kk");
        ...
        curl_close($curl);
        $responseData = json_decode($response, true);*/
        $url = 'http://open-safari.com/open?url=ticketby.co/';
    ...
    public function update_profile()
    {
        ...
        return view('front.userProfile', compact('user', 'languages', 'phone'));
    }
    ...
    public function userTickets()
    {
        ...
        $ticket['upcoming'] = Order::with(['orderchild', 'event:id,name,image,start_time,type,end_time,address', 'ticket:id,ticket_number,start_time,name,price,type', 'organization:id,external_id,first_name,last_name,image'])
        ...
        $ticket['past'] = Order::with(['event:id,name,image,start_time,type,end_time,address', 'ticket:id,ticket_number,name,type,price', 'organization:id,external_id,first_name,last_name,image'])
    ...
    {
        $data = Order::with(['event:id,name,image,start_time,type,end_time,address', 'ticket:id,ticket_number,name,price,type', 'organization:id,external_id,first_name,last_name,image'])->find($id);
  ...
  CURLOPT_POSTFIELDS => array('client_key' => 'a918ee4d-d87a-4298-8a80-04ee30d8bebc','order_id' => $order->order_id,'hash' =>$hash,'order_description' => 'Description','order_currency' => 'SAR','order_amount' => $request->payment,'card_number' => $request->card_input,'card_exp_month' =>$request->month,'card_exp_year' => '20'.$request->year,'card_cvv2' => $request->cvv,'payer_phone' => '+966559344333','payer_country' => 'SA','payer_address' => 'Abdulrhman.a@edfapay.com','action' => 'SALE','payer_zip' => '623524','payer_ip' => '176.44.76.100','payer_first_name' => 'Abdulrhman','payer_city' => 'Riyadh','auth' => 'N','payer_last_name' => 'alnafisah','payer_email' => 'Abdulrhman.a@edfapay.com','term_url_3ds' => 'https://ticketby.co/thankyou','recurring_init' => 'N','req_token' => 'N','merchant_origin' => 'http://pay.edfapay.com','card_scheme' => 'VISA'),
        ...
        if (isset($responseData['error_message'])) {
            $data['error_message'] = "Invalid Details or Details not accepted by payment gateway "; //$responseData['error_message'] . " - " . $responseData['errors'][0]['error_message'];
            $data['status'] = 400;
            return $data;
        }
    ...
    public function thankyou(Request $request)
    {
       ...
       // if ($order->payment_type == "TAMARA" && isset($order->tamara_order_id)) {
        ...
        $this->sendOrderMailPdf($orderId);
        \Log::error('order sucess ------'.$order->id);
        return view('frontend/thankyou');
    }

    public function failed()
    {
        // save token for sucess
    ...
    public function logicTesting(Request $request)
    {
        ...
        $PASSWORD = "a92f7b7f0d869d3e676c5facda5262ae";
        $card_number = Session::get('edfapay_card');
        $email = "alshammarisamar@gmail.com";
