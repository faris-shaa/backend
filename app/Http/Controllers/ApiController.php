...
class ApiController extends Controller
{
    ...
    public function eventDetailPost($id)
    {
        // echo "hi";
        // print_r ($request->all());
        //dd($request->date);
        ...
        foreach ($data->ticket as $key => $value) {
            ...
            {
                dd("kk");
                $order_id = Order::where('event_id', $value->event_id)->where('payment_status',1)->where('order_status','Complete')->whereDate('created_at',Carbon::parse($request->date)->format("Y-m-d"))->pluck('id')->toArray();
                $value->dateby_sold_ticket_count = OrderChild::whereIn('order_id',$order_id)->count();
            }
    ...
    {   
        ...
        foreach ($tickets as $key => $ticket_array) {
            ...
            {
                $event_orders =  Order::where('order_status',"Complete")->where('payment_status',1)->where('event_id',$request->event_id)->whereDate('created_at',Carbon::now()->format('Y-m-d'))->pluck('id')->toArray();
                $count_sold_out_ticekt = OrderChild::whereIn('order_id',$event_orders)->where('ticket_id',$ticket_array['ticket_id'])->count();
         ...
         $count_quantity = 0 ;
            foreach ($tickets as $key => $ticket_array) {
                ...
                for ($i = 1; $i <= $ticket_array['quantity']; $i++) {
                    ...
                    $ticket_checkin_data = Ticket::find($ticket_array['ticket_id']);
                    $child['order_id'] = $order->id;
                    $child['customer_id'] = $request->user_id;
                    $child['checkin'] = $ticket_checkin_data->maximum_checkins ?? null;
    ...
    public function sendOrderMail ( $id)
    {
        ...
        //posorder 
        $pos_order = PosOrder::where('order_id',$id)->first();
        if($pos_order)
        {
        ...
        if(is_array($request->date))
        {
            ...
            foreach ($request->date as $key => $date) {
                ...
                foreach ($time_slots as $key1 => $value_slot) {
                    ...
                    $slot_obj->start_time = Carbon::parse($value_slot->start_time)->format("h:i A");
                    $slot_obj->end_time = Carbon::parse($value_slot->end_time)->format("h:i A");
                    $slot_obj->date = $date;
