...
<div id="register_popup"
     class="hidden pop-modal fixed inset-0 flex items-center justify-center bg-dark bg-opacity-75 p-2 z-50  ">
    <div class="bg-dark_4  rounded-2xl   p-2  md:p-32-32 relative w-w-500">
        ...
        <div id="register-model" class="relative hidden form-container">
            ...
            <div id="errorMessages"
                 class=" errorMessages hidden text-center rounded-md mb-1 border border-red bg-red_light p-1 text-h8 ">
            ...
            <form id="registerForm" class=" max-w-lg">
                ...
                <div class=" mb-2">
                    <label for=""
                           class="text-gray_9 text-h7 md:text-h5 md:mb-3  mb-m6 block">{{__('Full name')}}</label>
                ...
                <div class="mb-2 font-12">
                    <label for=""
                           class="text-gray_9 text-h7 md:text-h5 md:mb-3  mb-m6 block">{{__('Mobile number')}}</label>
                ...
                <div class="mb-2">
                    <label for=""
                           class="text-gray_9 text-h7 md:text-h5 md:mb-3  mb-m6 block">{{__('Email address')}}</label>
    ...
    margin-right: 20px;" width="19" height="14" viewBox="0 0 19 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <button id="submitRegisterForm"
                        class="rounded-full bg-primary_color_8 px-p-32 py-1 md:p-12-24 md:w-full mx-auto block md:mt-8 mt-4 text-center text-h6 md:text-h5 font-medium w-full">{{__( 'Sign up')}}
                </button>
                <p class="h7 lg:h5 mt-3 text-gray_9">{{ __('By signing up you agree to our') }} <a href=""
                                                                                                   class="text-primary_color_6"> {{__('terms')}}</a>
                </p>
            </form>
            ...
            <div class="  f-bri mt-4 md:mt-7 text-center text-primary_color_6">
                ...
                <button data-form-user="login-model"
                        class="from-switch text-h6 lg:text-h5 font-medium ">{{__( 'sign in')}}</button>
            </div>
        </div>
        <!-- login popup -->
        <div id="login-model" class="relative hidden form-container  ">
            ...
            <form id="login_form" action="{{ url('/web/login') }}" method="POST" class=" min-w ">
                ...
                <div class="mb-2">
                    <label for=""
                           class="text-gray_9 text-h7 md:text-h5 md:mb-3  mb-m6 block">{{__('Mobile number or email')}}</label>
                    <div id="countrycontainr"
                         class="countrycontainr countriescd flex gap-1 border-gray_s rounded-lg border items-center  px-2"
                         style="margin-top: 10px;">

                        <!-- <div class="selectContainer"></div> -->
                        <input type="text" value="" name="user_name" required
                               placeholder="{{ __('Mobile number or email') }}"
                ...
                <button class="rounded-full bg-primary_color_8 px-p-32 py-1 md:p-12-24 md:w-full mx-auto block md:mt-8 mt-4 text-center text-h6 md:text-h5 font-medium"
                        style="width:100%"> {{__( 'Get code')}}
                </button>
            </form>
            <div>
                ...
                <div class="flex gap-7">
                    <a href="javascript:void(0)" id="Google-login"
                       class="bg-gray_35 rounded-lg md:rounded-2xl flex items-center gap-1 px-p-32 py-1 md:p-12-24 flex-1 justify-center text-h6 md:text-h5 font-medium">
            ...
            <div class="f-bri mt-4 md:mt-7 text-center text-primary_color_6">
                ...
                <button data-form-user="register-model"
                        class="from-switch text-h6 lg:text-h5 font-medium">{{__( 'sign up')}}</button>
            </div>
        </div>
        <!-- otp -->
        <div id="otp-model"
             class="hidden px-2 lg:px-0 pop-modal fixed inset-0 flex items-center justify-center bg-dark bg-opacity-75  z-30  ">
            <div class="bg-dark_4  rounded-2xl   p-2  md:p-32-32 relative w-w-500">
                    ...
                    <div class="flex items-center gap-1">
                        <button id="returnTosign" class="@if($lang == 'ar') rotate-180 @endif">
                            <svg width="11" height="19" viewBox="0 0 11 19" fill="none"
                                ...
                                <path d="M0.601562 8.64062L8.10156 1.14062C8.57031 0.632812 9.39062 0.632812 9.85938 1.14062C10.3672 1.60938 10.3672 2.42969 9.85938 2.89844L3.25781 9.5L9.85938 16.1406C10.3672 16.6094 10.3672 17.4297 9.85938 17.8984C9.39062 18.4062 8.57031 18.4062 8.10156 17.8984L0.601562 10.3984C0.09375 9.92969 0.09375 9.10938 0.601562 8.64062Z"
                                      fill="#C4ACD3"/>
                            </svg>
                        </button>
                ...
                <form id="verificationForm" class="verification">
                    ...
                    <div id="verification" class="flex @if($lang == 'ar') flex-row-reverse @endif">
                        <input type="number" autofocus maxlength="1"
                               class=" text-center verification-pass h-16 w-16 col-span-1 mx-auto outline-0 focus:border-primary_color_6 text-white bg-transparent border border-gray_s p-16-16 rounded-lg"
                               oninput="moveToNext(this, 1)" data-val="p-1">
                        <input type="number" maxlength="1"
                               class="disabled text-center verification-pass h-16 w-16 col-span-1 mx-auto outline-0 focus:border-primary_color_6 text-white bg-transparent border border-gray_s p-16-16 rounded-lg"
                               oninput="moveToNext(this, 2)" data-val="p-2">
                        <input type="number" maxlength="1"
                               class="disabled text-center verification-pass h-16 w-16 col-span-1 mx-auto outline-0 focus:border-primary_color_6 text-white bg-transparent border border-gray_s p-16-16 rounded-lg"
                               oninput="moveToNext(this, 3)" data-val="p-3">
                        <input type="number" maxlength="1"
                               class="disabled text-center verification-pass h-16 w-16 col-span-1 mx-auto outline-0 focus:border-primary_color_6 text-white bg-transparent border border-gray_s p-16-16 rounded-lg"
                               oninput="moveToNext(this, 4)" data-val="p-4">
                    </div>
                    <p class="error" id="login-error"></p>
                    <button type="button" id="confirmButton"
                            class="rounded-full bg-primary_color_8 p-12-24 w-full block mt-7 text-center">{{__('Confirm')}}</button>
                </form>
            </div>
        </div>
        <!--  password popup -->
        <div id="password-model" class="hidden form-container">
            ...
            <form id="resetPasswordForm">
                ...
                <button id="submitResetForm"
                        class="rounded-full bg-primary_color_8 p-12-24 w-full block md:mt-8 mt-4 text-center text-h6 md:text-h5">{{ __('Reset Password') }}
                </button>
            </form>
