...
<div class="container mt-12 md:mt-16 ">
   <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
      <div class="col-span-12 md:col-span-8  ">
         ...
         <div class="">
            ...
            <p class="flex gap-2  mt-2 lg:mt-2 items-center">
               ...
               <span class="h5 ">
                  ...
                  @endif
                  @else
                  {{ Carbon\Carbon::parse( $event['event']['start_time'])->format('d') }}
                  @if (session('direction') == 'rtl')
                  {{ Carbon\Carbon::parse($event['event']['start_time'])->locale('ar')->translatedFormat('M Y') }}
                  {{ Carbon\Carbon::parse($event['event']['start_time'])->locale('ar')->translatedFormat('h:i A') }}
                  @else
                  {{ Carbon\Carbon::parse($event['event']['start_time'])->format('M Y') }}
         ...
         <!--  <div class="mt-4 flex gap-2 flex-wrap">
              <a href="{{ route("events.organizer", [request("id"), request("name")]) }}">
                  <small>
                      <stron>{{ __("By") }}</stron>
...
<div class="container mt-12 md:mt-32">
   ...
   <div class="bg-light bg-opacity-5 rounded-2xl border border-primary_color_o10_1  p-2  md:p-32-24 mt-4 lg:mt-8">
      ...
      <p class="h5 lg:h4 mt-4 paragraph-3 max-h-96 overflow-x-auto ">
         {!! $lang == 'ar' ? $event['event']['description_arabic'] : $event['event']['description']  !!}
      </p>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-16">
         ...
         <div class="bg-primary_color_12  hover:border-primary_color_7 transition-all rounded-2xl border border-primary_color_o25_8  p-2  md:p-32-24 text-center">
            ...
            <div class="flex mt-4 items-center justify-center gap-4 pro-qty">
               <button type="button" class="disable decrement opacity-25 qtybtn" data-ticket-price="{{ $item->price }}" data-ticket-id="{{ $item->id }}">
                  <svg width="33" height="32" viewBox="0 0 33 32" fill="none" xmlns="http://www.w3.org/2000/svg">
               ...
               <button type="button" class="increment " data-ticketAvailabl="{{$item->available_qty}}" data-ticket-price="{{ $item->price }}" data-ticket-id="{{ $item->id }}">
                  <svg width="33" height="32" viewBox="0 0 33 32" fill="none" xmlns="http://www.w3.org/2000/svg">
                     <rect x="1" y="0.5" width="31" height="31" rx="7.5" fill="#FBF9FD" stroke="#723995" />
                     <path d="M9.5 16H23.5" stroke="#723995" stroke-width="2" stroke-linecap="round" />
            ...
            <div style="margin-top: 20px; color:red;" class="font-poppins font-medium text-base leading-7 text-danger" id="quantityMsg"></div>
         </div>
         @endif
         @endforeach
      </div>
