...
class FrontendController extends Controller
{
    ...
    public function eventDetail($id, $name = null)
    {
        ...
        $data->time_slots = EventTime::where('event_id',$id)->get();

        $data->free_ticket = Ticket::where([['event_id', $data->id], ['is_deleted', 0], ['type', 'free'], ['status', 1], ['end_time', '>=', $date->format('Y-m-d H:i:s')], ['start_time', '<=', $date->format('Y-m-d H:i:s')]])->orderBy('id', 'ASC')->get();
        $data->paid_ticket = Ticket::where([['event_id', $data->id], ['is_deleted', 0], ['type', 'paid'], ['status', 1], ['end_time', '>=', $date->format('Y-m-d H:i:s')], ['start_time', '<=', $date->format('Y-m-d H:i:s')]])->orderBy('id', 'ASC')->get();
    ...
    public function createOrder(Request $request)
    {
        ...
        { 
            ...
            ->post('https://api.edfapay.com/applepay/orders/s2s/sale', [
                ...
                'order_currency' => 'SAR',
                'order_description' => 'Order No'.$order_id,
                ...
                'payer_address2' => 'Riyadh',
                'payer_middle_name' => $request->name,
            ]);  
            ...
            \Log::info('Payment Response:',$response->json());
        ...
        $data['payment'] = $order_amount;  
        $data['payment_token'] = $publicKeyHash;
        ...
        $data['seat_details'] = isset($request->selectedSeats) ? $request->selectedSeats : null;
         $data['time_slot_id'] = isset($request->time_slot_id) ? $request->time_slot_id : null;
        $data['event_book_date'] = isset($request->event_book_date) ? $request->event_book_date : null;
        ...
        for ($i = 1; $i <= $request->quantity; $i++) {
            ...
            $child['paid'] = $data['payment_status'];
  ...
  CURLOPT_POSTFIELDS => array('client_key' => 'a918ee4d-d87a-4298-8a80-04ee30d8bebc','order_id' => $order->order_id,'hash' =>$hash,'order_description' => 'Description','order_currency' => 'SAR','order_amount' => $request->payment,'card_number' => $request->card_input,'card_exp_month' =>$request->month,'card_exp_year' => '20'.$request->year,'card_cvv2' => $request->cvv,'payer_phone' => '+966559344333','payer_country' => 'SA','payer_address' => 'Abdulrhman.a@edfapay.com','action' => 'SALE','payer_zip' => '623524','payer_ip' => '176.44.76.100','payer_first_name' => 'Abdulrhman','payer_city' => 'Riyadh','auth' => 'N','payer_last_name' => 'alnafisah','payer_email' => 'Abdulrhman.a@edfapay.com','term_url_3ds' => 'https://ticketby.co/thankyou','recurring_init' => 'N','req_token' => 'N','merchant_origin' => 'http://pay.edfapay.com','card_scheme' => 'VISA'),
    ...
    public function thankyou ()
   {
    ...
    public function sendOTPMail($dataMail)
    {
        ...
        Mail::send(['html'=>'frontend.email.otp'], $data, function($message) use ($data) {
           $message->to("hivasavada£gmail.com")->subject
              ('OTP Verification');
           $message->from('ticketbyksa@gmail.com','TicketBy');
        });
    }
    ...
    public function emailtesting ( )
    {
        $data = array();
            ...
            $message->to( "kabirphp099@gmail.com")->subject
                ('OTP Verification');
    ...
    public function createOrderTickets ()
    {
        ...
        $order_data['event_id'] = 148;
        $order_data['customer_id'] = 108;
        $order_data['organization_id'] = 108;
        $order_data['order_status'] = 'Pending';
        $order_data['order_status'] = 'Pending';
        $order_data['ticket_id'] = 176 ; // 90 f 91 m
        $order_data['quantity'] = 50 ; // 90 f 91 m
        $order_data['tax'] = 0 ; 
    ...
    public function eventTimeCheck ( Request $request )
    {
        ...
        foreach ($tickets as $key => $value) {
            $ticket_array[$value->id] = $value->quantity - Order::where('ticket_id',$value->id)->where('time_slot_id',$request->event_time_id)->where('event_book_date',$request->date)->sum('quantity');  
        }
    ...
    public function getTimeSlot(Request $request) {
        ...
        $event_time_slot = EventTime::where('event_id',$request->item_id)->where('ticket_id',$request->ticket_id)->get();    

        // Fetch available time slots based on the date and item ID
        // This is just a mockup; replace with your actual logic
        /*$timeSlots = [
            '09:00 AM - 10:00 AM',
            '10:00 AM - 11:00 AM',
            '11:00 AM - 12:00 PM',
        ...
        return response()->json($event_time_slot);
    }
